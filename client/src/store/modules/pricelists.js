...
export const actions = {
      ...
      .then((response) => {
        if (response.status === 200) {
          ...
          list.forEach((el) => {
            ...
            el.presentation = `Cennik nr ${el.number} od ${el.createdAt}`
          })
  ...
  async findByPk({ commit }, payload) {
    ...
    return axios
      .get(`/pricelist/${payload.params.id}`, payload.query)
      .then((response) => {
  ...
  async update({ commit }, payload) {
    return axios
      .put(`/pricelist/${payload.id}`, payload)
      .then((response) => {
        return response
      })
      .catch((error) => {
...
export const getters = {
  ...
  presentation: (state) => (viewId) => {
    ...
    let presentation = ''
    if (objectView) {
      presentation = objectView.object.priceCode
    }
