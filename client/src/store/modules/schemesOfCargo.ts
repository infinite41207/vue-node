...
import { findAll, findByPk, createItem, updateItem, addNewItem, changeDeletionMark, deleteItem, resetState } from '../service/actions'

import BasicObject from '@/dto/SchemeOfCargo.json'
...
export const actions = {
  ...
  async getListFields() {
    return [
      { key: 'name', label: 'table.name', sortable: true, width: '67%' },
      { key: 'code', label: 'table.code', sortable: true, width: '20%' },
      { key: 'typeOfOperation', label: 'table.operationType', sortable: true, width: '20%' },
