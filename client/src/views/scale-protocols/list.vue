...
import ScaleProtocol from '@/dto/ScaleProtocol.json'
import Question from '@/components/common/question'
import { uuid } from 'vue-uuid'

const namespace = 'scaleProtocols'
...
export default {
  ...
  page() {
    return {
      title: this.$t('route.scaleProtocols'),
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'type', label: this.$t('table.type'), sortable: true },
        { key: 'markedToDelete', label: this.$t('table.markedToDelete'), sortable: true },
  ...
  computed: {
    ...mapGetters({
      listView: `scaleProtocols/listView`,
    }),
  ...
  methods: {
    ...mapMutations({
      addObjectView: `scaleProtocols/addObjectView`,
      setListViewProperty: `scaleProtocols/setListViewProperty`,
      setFilter: `scaleProtocols/setFilters`,
      setSort: `scaleProtocols/setSort`,
    }),
    ...
    async updateList() {
      ...
      await this.$store.dispatch(`scaleProtocols/findAll`, filterStr)
    },
    ...
    async deleteRestoreItemEnd(itemId) {
      await this.$store.dispatch('scaleProtocols/changeDeletionMark', {
        id: itemId,
      })
    ...
    addNewObject() {
      const viewId = uuid.v4()
      const object = _.cloneDeep(ScaleProtocol)
      object.id = viewId
      object.isNew = true

      this.addObjectView({ viewId, object })
      this.$router.push({ name: 'scale-protocol-detail', params: { id: viewId } })
    },

    async editObject(objectId) {
      await this.$store
        .dispatch(`scaleProtocols/findByPk`, {
          params: {
            id: objectId,
          },
        })
        .then((response) => {
          if (response.status === 200) {
            this.$router.push({ name: 'scale-protocol-detail', params: { id: objectId } })
          }
        })
        .catch((error) => {
          console.error(error)
