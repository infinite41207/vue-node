          ...
          <b-row>
            <b-col class="overflow-x-auto">
              ...
              >
                ...
                <template v-slot:cell(initDate)="data">
                  <span>{{ convertDate(data.item.initDate) }}</span>
                </template>
...
import Warehouses from '@/dto/Warehouses.json'
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'code', label: this.$t('table.code'), sortable: true, width: '20%' },
        { key: 'initDate', label: this.$t('table.initDate'), sortable: true },
  ...
  methods: {
    ...mapMutations({
      addObjectView: `warehouses/addObjectView`,
      setListViewProperty: `warehouses/setListViewProperty`,
      setFilter: `warehouses/setFilters`,
      setSort: `warehouses/setSort`,
    }),
    ...
    async updateList() {
      ...
      await this.$store.dispatch(`warehouses/findAll`, filterStr)
    },
    ...
    async deleteRestoreItemEnd(itemId) {
      await this.$store.dispatch('warehouses/changeDeletionMark', {
        id: itemId,
      })
    ...
    addNewObject() {
      const viewId = uuid.v4()
      const object = _.cloneDeep(Warehouses)
      object.id = viewId
      object.isNew = true

      this.addObjectView({ viewId, object })
      this.$router.push({ name: 'warehouse-detail', params: { id: viewId } })
    },

    async editObject(objectId) {
      await this.$store
        .dispatch(`warehouses/findByPk`, {
          params: {
            id: objectId,
          },
        })
        .then((response) => {
          if (response.status === 200) {
            this.$router.push({ name: 'warehouse-detail', params: { id: objectId } })
          }
        })
        .catch((error) => {
          console.error(error)
