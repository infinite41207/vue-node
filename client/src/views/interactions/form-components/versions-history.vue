...
<script>
export default {
  name: 'InteractionVersionHistory',
  props: {
    versionUuid: {
      type: String,
      required: true,
  ...
  data() {
    return {
      versionsFields: [
        ...
        { key: 'executor.name', label: this.$t('table.executor') },
  ...
  async mounted() {
    ...
    const response = await this.$store.dispatch('interactions/getVersions', {
      params: {
        filter: {
    ...
    if (response.status === 200) {
      this.versionsTable = response.data
    }
  ...
  methods: {
    ...
    async editItem(itemId) {
      ...
      if (response.status === 200) {
        this.$router.push({ name: 'interactions-detail', params: { id: itemId } })
