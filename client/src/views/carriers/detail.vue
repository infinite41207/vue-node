      ...
      <b-row>
        <b-col md="6">
          <b-form-group horizontal :label-cols="3" :label="$t('table.name')" label-for="item-name">
            <b-form-input id="item-name" v-model="name" type="text" name="item-name" size="sm" :state="!$v.name.$invalid"> </b-form-input>
          </b-form-group>
        </b-col>
      </b-row>
    </b-card>
...
export default {
  ...
  page() {
    return { title: this.$t('route.carrier'), meta: [{ name: 'description', content: appConfig.description }] }
  },
