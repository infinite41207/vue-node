<template>
  <Layout>
    ...
    <b-modal v-model="showItemForm" title="Edycja grupy statusów" title-class="font-18" hide-footer>
      ...
      <b-form-group label="Kolejność" label-for="itemSortId">
        <b-form-input id="itemSortId" v-model="currentItem.sortId" type="text" name="item-sort-id" size="sm"></b-form-input>
      </b-form-group>
...
export default {
  name: 'ReclStatusGroupsList',

  page: {
    title: 'Grupy statusów',
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'sortId', label: 'Kolejność', sortable: true },
  ...
  methods: {
    ...
    async initialize() {
      this.itemsData = await this.$store.dispatch('reclamationStatusGroups/findAll', {})
    ...
    async saveChanges() {
      if (this.currentItem.id !== null) {
        const res = await this.$store.dispatch('reclamationStatusGroups/updateItem', this.currentItem)
      } else {
        const res = await this.$store.dispatch('reclamationStatusGroups/addItem', this.currentItem)
      }
      this.initialize()
    ...
    async deleteItem(deleteItem) {
      const res = await this.$store.dispatch('reclamationStatusGroups/deleteItem', deleteItem)
      this.initialize()
      this.currentItem = { ...this.newItem() }
