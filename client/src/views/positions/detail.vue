...
export default {
  ...
  computed: {
    ...mapGetters({
      getObjectView: 'positions/objectView',
    }),
  ...
  methods: {
    ...mapMutations({
      setObjectViewProperty: 'positions/setObjectViewProperty',
      setObjectProperty: 'positions/setObjectProperty',
      delObjectView: 'positions/delObjectView',
    }),
    ...
    async writeObject() {
      ...
      if (this.object.isNew) {
        response = await this.$store.dispatch('positions/create', this.object).catch((error) => {
          console.error(error)
        })
      } else {
        response = await this.$store.dispatch('positions/update', this.object).catch((error) => {
          console.error(error)
        })
      }
    ...
    async closeView() {
      ...
      await this.$router.push({ name: 'positions-list' })
    },
  },
}
</script>
