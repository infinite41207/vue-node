            ...
            <b-col>
              <b-button-toolbar>
                <b-button-group>
                  ...
                  <b-button variant="outline-secondary" class="btn-sm ml-2" @click="openHistory">
...
import Assortments from '@/dto/Assortments.json'
import Question from '@/components/common/question'
import { uuid } from 'vue-uuid'

const namespace = 'paymentTypes'
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'currency.name', label: this.$t('table.currency'), sortable: true },
  ...
  methods: {
    ...
    type(data) {
      const findEl = this.typesOfPayment.find((el) => el.value === data.value )
      return findEl.name
    },

    status(data) {
      const findEl = this.statusList.find((el) => el.value === data.value )
      return findEl.name
    },
    ...
    addNewObject() {
      const viewId = uuid.v4()
      const object = _.cloneDeep(Assortments)
      object.id = viewId
      object.isNew = true
