<template>
  <Layout>
    ...
    <b-modal v-model="showItemForm" title="Edycja typu zlecenia" title-class="font-18" hide-footer>
      ...
      <b-form-group label="Dni" label-for="days">
        <b-form-input id="days" v-model="currentItem.days" type="number" name="days-id" size="sm"></b-form-input>
      </b-form-group>
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'days', label: 'Dni', sortable: true },
  ...
  methods: {
    ...
    async initialize() {
      this.itemsData = await this.$store.dispatch('reclamationTypes/findAll', {})
    ...
    async saveChanges() {
      if (this.currentItem.id !== null) {
        const res = await this.$store.dispatch('reclamationTypes/update', this.currentItem)
      } else {
        const res = await this.$store.dispatch('reclamationTypes/create', this.currentItem)
      }
      this.initialize()
    ...
    async deleteItem(deleteItem) {
      const res = await this.$store.dispatch('reclamationTypes/delete', deleteItem)
      this.initialize()
      this.currentItem = { ...this.newItem() }
