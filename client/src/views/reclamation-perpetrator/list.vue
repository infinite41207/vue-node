<template>
  <Layout>
    ...
    <b-modal v-model="showItemForm" title="Edycja elementu" title-class="font-18" hide-footer>
...
export default {
  ...
  data() {
    return {
      title: 'Odpowiedzialni za reklamacje',
  ...
  methods: {
    ...
    async initialize() {
      this.itemsData = await this.$store.dispatch('reclamationPerpetrators/findAll', {})
    ...
    async saveChanges() {
      if (this.currentItem.id !== null) {
        const res = await this.$store.dispatch('reclamationPerpetrators/updateItem', this.currentItem)
      } else {
        const res = await this.$store.dispatch('reclamationPerpetrators/addItem', this.currentItem)
      }
      this.initialize()
    ...
    async deleteItem(deleteItem) {
      const res = await this.$store.dispatch('reclamationPerpetrators/deleteItem', deleteItem)
      this.initialize()
      this.currentItem = { ...this.newItem() }
