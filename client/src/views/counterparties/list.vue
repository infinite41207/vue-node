              ...
              >
                ...
                <template v-slot:cell(status)="data">
                  <span
                    class="badge"
                    :class="{
                      'badge-success': data.item.status === 'Active',
                      'badge-danger': data.item.status === 'Deactive',
                    }"
                    >{{ data.item.status ? $t(`common.${data.item.status.toLowerCase()}`) : '' }}</span
                  >
                </template>
...
import Counterparty from '@/dto/Customer.json'
import Question from '@/components/common/question'
import { uuid } from 'vue-uuid'

const namespace = 'counterparties'
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        { key: 'name', label: this.$t('table.name'), sortable: true, width: '30%' },
        { key: 'nip', label: this.$t('table.nip'), sortable: true, width: '5%' },
        { key: 'abbreviation', label: this.$t('table.abbreviation'), sortable: true, width: '3%' },
        { key: 'country.name', label: this.$t('table.country'), sortable: true, width: '10%' },
        { key: 'region', label: this.$t('table.region'), sortable: true, width: '10%' },
        { key: 'address', label: this.$t('table.address'), sortable: true },
        { key: 'status', label: this.$t('table.status'), sortable: true, width: '3%' },
  ...
  methods: {
    ...
    addNewObject() {
      const viewId = uuid.v4()
      const object = _.cloneDeep(Counterparty)
      object.id = viewId
      object.isNew = true
