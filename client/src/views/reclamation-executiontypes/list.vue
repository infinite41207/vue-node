...
export default {
  name: 'ReclExecutionTypeList',

  page: {
    title: 'Typy realizacji zleceń',
  ...
  methods: {
    ...
    async initialize() {
      this.itemsData = await this.$store.dispatch('reclamationExecutionTypes/findAll', {})
    ...
    async saveChanges() {
      if (this.currentItem.id !== null) {
        const res = await this.$store.dispatch('reclamationExecutionTypes/updateItem', this.currentItem)
      } else {
        const res = await this.$store.dispatch('reclamationExecutionTypes/addItem', this.currentItem)
      }
      this.initialize()
    ...
    async deleteItem(deleteItem) {
      const res = await this.$store.dispatch('reclamationExecutionTypes/deleteItem', deleteItem)
      this.initialize()
      this.currentItem = { ...this.newItem() }
