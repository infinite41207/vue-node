<template>
  <Layout>
        ...
        <b-card>
          ...
          <b-row>
            <b-col>
              ...
              >
                <template v-slot:cell(yearNumber)="data">
                  <a href="javascript:void(0);" @click="editItem(data.item)"
                    ><p>{{ data.item.yearNumber }}</p>
                  </a>
                </template>
    ...
    <b-modal v-model="showItemForm" title="Edycja numeratora" title-class="font-18" hide-footer>
      <b-form-group label="Rok" label-for="yearNumber">
        <b-form-input id="yearNumber" v-model="currentItem.yearNumber" type="number" name="item-year-number" size="sm"></b-form-input>
      </b-form-group>

      <b-form-group label="Ostatni numer" label-for="lastNumber">
        <b-form-input id="lastNumber" v-model="currentItem.lastNumber" type="number" name="item-last-number" size="sm"></b-form-input>
      </b-form-group>

      <div class="text-right pt-2 pb-2">
        <b-button variant="light" @click="cancelItemModification">Anuluj</b-button>
        <b-button type="submit" variant="success" class="ml-2" @click="saveChanges">Zapisz zmiany</b-button>
      </div>
    </b-modal>
  </Layout>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'yearNumber', label: 'Rok', sortable: true },
        { key: 'lastNumber', label: 'Ostatni numer', sortable: false },
  ...
  methods: {
    ...
    async initialize() {
      this.itemsData = await this.$store.dispatch('documentNumerators/findAll', {})
    ...
    async saveChanges() {
      if (this.currentItem.id !== null) {
        const res = await this.$store.dispatch('documentNumerators/updateItem', this.currentItem)
      } else {
        const res = await this.$store.dispatch('documentNumerators/addItem', this.currentItem)
      }
      this.initialize()
    ...
    async deleteItem(deleteItem) {
      await this.$store.dispatch('documentNumerators/deleteItem', deleteItem)
