<template>
  <Layout>
            ...
            <b-col>
              ...
              >
                ...
                <template v-slot:cell(delete)="data">
                  <a href="javascript:void(0);" class="ri-delete-bin-7-fill text-danger" @click="beforeDeleteItem(data.item)"> </a>
                </template>
              </b-table>
            </b-col>
          </b-row>
    ...
    <b-modal v-model="showItemForm" title="Edycja tagu" title-class="font-18" hide-footer>
...
export default {
  ...
  data() {
    return {
      title: this.$t('route.interactionTags'),
  ...
  methods: {
    ...
    async initialize() {
      await this.$store
        .dispatch('interactionTags/findAll', {})
        .then((response) => {
    ...
    async saveChanges() {
      ...
      if (this.currentItem.id !== null) {
        await this.$store.dispatch('interactionTags/updateItem', saveItem)
      } else {
        await this.$store.dispatch('interactionTags/addItem', saveItem)
      }
      this.initialize()
    ...
    async deleteItem(deleteItem) {
      await this.$store.dispatch('interactionTags/deleteItem', deleteItem)
