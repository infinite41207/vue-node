...
import UnitsOfMeasure from '@/dto/UnitsOfMeasure.json'
import Question from '@/components/common/question'
import { uuid } from 'vue-uuid'

const namespace = 'unitsOfMeasure'
...
export default {
  ...
  page() {
    return {
      title: this.$t('route.unitsOfMeasure'),
  ...
  computed: {
    ...mapGetters({
      listView: `unitsOfMeasure/listView`,
    }),
  ...
  methods: {
    ...mapMutations({
      addObjectView: `unitsOfMeasure/addObjectView`,
      setListViewProperty: `unitsOfMeasure/setListViewProperty`,
      setFilter: `unitsOfMeasure/setFilters`,
      setSort: `unitsOfMeasure/setSort`,
    }),
    ...
    async updateList() {
      ...
      await this.$store.dispatch(`unitsOfMeasure/findAll`, filterStr)
    },
    ...
    async deleteRestoreItemEnd(itemId) {
      await this.$store.dispatch('unitsOfMeasure/changeDeletionMark', {
        id: itemId,
      })
    ...
    addNewObject() {
      const viewId = uuid.v4()
      const object = _.cloneDeep(UnitsOfMeasure)
      object.id = viewId
      object.isNew = true

      this.addObjectView({ viewId, object })
      this.$router.push({ name: 'unit-of-measure-detail', params: { id: viewId } })
    },

    async editObject(objectId) {
      await this.$store
        .dispatch(`unitsOfMeasure/findByPk`, {
          params: {
            id: objectId,
          },
        })
        .then((response) => {
          if (response.status === 200) {
            this.$router.push({ name: 'unit-of-measure-detail', params: { id: objectId } })
          }
        })
        .catch((error) => {
          console.error(error)
