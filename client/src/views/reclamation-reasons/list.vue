<template>
  <Layout>
    ...
    <b-modal v-model="showItemForm" title="Edycja przyczyny reklamacji" title-class="font-18" hide-footer>
...
export default {
  ...
  methods: {
    ...
    async initialize() {
      this.itemsData = await this.$store.dispatch('reclamationReasons/findAll', {})
    ...
    async saveChanges() {
      if (this.currentItem.id !== null) {
        const res = await this.$store.dispatch('reclamationReasons/updateItem', this.currentItem)
      } else {
        const res = await this.$store.dispatch('reclamationReasons/addItem', this.currentItem)
      }
      this.initialize()
    ...
    async deleteItem(deleteItem) {
      const res = await this.$store.dispatch('reclamationReasons/deleteItem', deleteItem)
      this.initialize()
      this.currentItem = { ...this.newItem() }
