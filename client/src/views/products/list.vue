...
import Product from '@/dto/Product.json'
...
export default {
  ...
  data() {
    return {
      title: this.$t(`route.${namespace}`),
      fields: [
        { key: 'name', label: this.$t('table.name'), sortable: true, width: '50%' },
        { key: 'code', label: this.$t('table.code'), sortable: true, width: '10%' },
        { key: 'article', label: this.$t('table.article'), sortable: true, width: '10%' },
        { key: 'unitOfMeasure.name', label: this.$t('table.jOM'), sortable: true, width: '5%' },
  ...
  methods: {
    ...
    addNewObject() {
      const viewId = uuid.v4()
      const object = _.cloneDeep(Product)
      object.img = require(`@/assets/images/products/noimg.png`)
      object.id = viewId
      object.isNew = true
