<template>
  <Layout>
    ...
    <b-modal v-model="showItemForm" title="Edycja statusu" title-class="font-18" hide-footer>
      ...
      <b-form-checkbox v-model="currentItem.isClosed" name="is-closed" switch>
        {{ $t('table.isClosed') }}
      </b-form-checkbox>

      <b-form-group label="Nazwa zdefiniowana" class="mt-2" label-for="item-predefined-name">
        <b-form-input id="item-predefined-name" v-model="currentItem.predefinedName" type="text" name="item-item-predefined-name" size="sm"></b-form-input>
      </b-form-group>
...
export default {
  ...
  data() {
    return {
      title: this.$t('route.interactionStatuses'),
      ...
      fields: [
        ...
        { key: 'color', label: 'Kolor', sortable: true },
  ...
  methods: {
      ...
      await this.$store
        ...
        .then((response) => {
          if (response && response.status === 200) {
            this.itemsData = response.data.responseData
    ...
    async saveChanges() {
      ...
      if (this.currentItem.id !== null) {
        await this.$store.dispatch('interactionStatuses/updateItem', saveItem)
      } else {
        await this.$store.dispatch('interactionStatuses/addItem', saveItem)
      }
      this.initialize()
    ...
    async deleteItem(deleteItem) {
      await this.$store.dispatch('interactionStatuses/deleteItem', deleteItem)
