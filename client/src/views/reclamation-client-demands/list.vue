<template>
  <Layout>
    ...
    <b-modal v-model="showItemForm" title="Edycja typu zlecenia" title-class="font-18" hide-footer>
...
export default {
  name: 'ReclClientDemandList',

  page: {
    title: 'Oczekiwania klienta',
  ...
  methods: {
    ...mapMutations({
      addObjectView: `reclamationClientDemands/addObjectView`,
      setListViewProperty: `reclamationClientDemands/setListViewProperty`,
      setFilter: `reclamationClientDemands/setFilters`,
      setSort: `reclamationClientDemands/setSort`,
    }),
    ...
    async updateList() {
      ...
      await this.$store.dispatch(`reclamationClientDemands/findAll`, filterStr)
    },
    ...
    async saveChanges() {
      if (this.currentItem.id !== null) {
        const res = await this.$store.dispatch('reclamationClientDemands/update', this.currentItem)
      } else {
        const res = await this.$store.dispatch('reclamationClientDemands/create', this.currentItem)
      }
      this.initialize()
    ...
    async deleteItem(deleteItem) {
      const res = await this.$store.dispatch('reclamationClientDemands/delete', deleteItem)
      this.initialize()
      this.currentItem = { ...this.newItem() }
  ...
  computed: {
    ...mapGetters({
      listView: `reclamationClientDemands/listView`,
    }),
  },
}
</script>
