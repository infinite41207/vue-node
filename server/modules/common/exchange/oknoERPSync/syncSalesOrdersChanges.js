...
module.exports = async () => {
  ...
  const salesOrdersLoadedList = []

  const queryResult = await exchangeOknoERP
    .getSalesOrdersChanges()
    .then(async (queryResult) => {
      for (let item of queryResult) {
        ...
        if (customer) {
          ...
          if (salesOrder) {
            await SalesOrder.update(salesOrderData, { where: { id: salesOrder.id } })
              .then(async () => {
                console.log('Update sales order: ', salesOrderData.numberStr, '-->')
                salesOrdersLoadedList.push({ uuid: salesOrderData.uuid })
              })
              .catch((err) => {
                console.log(err)
                //logger.error('Error in syncSalesOrdersChanges update SalesOrder', { meta: err });
              })
          } else {
            await SalesOrder.create(salesOrderData)
              .then(async () => {
                console.log('Create sales order: ', salesOrderData.numberStr, '-->')
                salesOrdersLoadedList.push({ uuid: salesOrderData.uuid })
              })
              .catch((err) => {
                console.log(err)
                //logger.error('Error in syncSalesOrdersChanges create SalesOrder', { meta: err });
              })
          }
        } else {
    ...
    .catch((err) => {
      console.error('Problem synchronizacji zamówień klientów:', err)
    })

  await exchangeOknoERP.deleteSalesOrdersChanges({ uuids: salesOrdersLoadedList }).catch((err) => {
    console.error('Problem z usunięciem z rejestracji zamówień klientów:', err)
  })
